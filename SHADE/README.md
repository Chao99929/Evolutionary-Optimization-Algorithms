...
```sh
./shade function runs max_evaluations dimension population history
```
...
The terminal prints best fitness and its running average. `output.txt` is overwritten each run and retains the last run. Full generations are processed, so the trial evaluation limit may be exceeded.
...
This removes the compiled `shade` executable.
